...
sequenceDiagram
    ...
    participant AI Engine (Gemini/OpenAI)
    participant WebContainer

    User->>Frontend (React): Enter prompt or write code
    Frontend->>AI Engine: Send code/text input
    AI Engine-->>Frontend (React): Return suggestions/website layout
    User->>Frontend (React): Click "Run"
    Frontend->>WebContainer: Execute environment in-browser
    WebContainer-->>Frontend (React): Display live results
    Frontend->>Backend (Express): Save/Deploy project
    Backend-->>User: Confirmation + Deployment link
```
