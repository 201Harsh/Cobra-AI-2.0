...
const DevDash = ({ activeSection }: { activeSection: string }) => {
  ...
  const GetCurrentVenomLab = async () => {
    try {
      ...
      if (res.status === 200) {
        setLabData(res.data.lab);
        // Simulate AI usage data based on lab creation date
        const createdDate = new Date(res.data.lab.createdAt);
        const daysSinceCreation = Math.floor((Date.now() - createdDate.getTime()) / (1000 * 60 * 60 * 24));
        const totalHours = Math.min(daysSinceCreation * 2, 150);
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl border border-red-500/20 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-400">Lab Name</label>
              <div className="flex items-center gap-3 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <span className="text-2xl">{getEnvironmentIcon(labData?.environment || "")}</span>
                <span className="text-white font-semibold">{labData?.name || "Unnamed Lab"}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <span className="text-white">{labData?.creator || "Unknown"}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <span className="text-white">{getEnvironmentName(labData?.environment || "")}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <span className="text-white">
                  {labData?.createdAt ? new Date(labData.createdAt).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  }) : "Unknown"}
            ...
            <div className="space-y-2">
              ...
              <div className="p-3">
                <span className={`px-3 py-1 rounded-full text-sm border ${getStatusColor(labData?.status || "")}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <span className="text-white">{labData?.members?.length || 1} members</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* AI Usage & Billing */}
        <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl border border-red-500/20 p-6">
          <h2 className="text-2xl font-bold text-white mb-6">AI Usage & Billing</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-semibold">{aiUsage.totalHours}h</span>
                </div>
                
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400">Monthly Usage</span>
                    <span className="text-white font-semibold">{aiUsage.monthlyUsage}h / {aiUsage.monthlyLimit}h</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div 
                      className="bg-gradient-to-r from-red-500 to-pink-500 h-2 rounded-full transition-all duration-500"
                      style={{ width: `${(aiUsage.monthlyUsage / aiUsage.monthlyLimit) * 100}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-700">
                  ...
                  <span className="text-white font-semibold">${aiUsage.cost.toFixed(2)}</span>
                </div>
              </div>
            </div>
